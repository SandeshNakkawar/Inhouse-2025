            ...
            {batches.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {/* {batches.map((batch) => (
                  <div 
                    key={batch.id} 
                    className="border rounded-lg p-4 hover:shadow-md cursor-pointer transition-shadow hover:bg-[#155E95] hover:text-white"
                    onClick={() => handleBatchClick(batch)}
                  >
                    <div className="flex justify-between items-start mb-2 hover:text-white">
                      <h3 className="text-lg font-medium hover:text-white">{batch.name}</h3>
                      <span className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded hover:text-white">
                        {batch.year}
                      </span>
                    </div>
                    <div className="space-y-2 text-sm text-gray-600 hover:text-white">
                ...
                {batches.map((batch) => (
                    <div 
                      key={batch.id} 
                      className="group border rounded-lg p-4 hover:shadow-md cursor-pointer transition-shadow hover:bg-[#155E95] hover:text-white"
                      onClick={() => handleBatchClick(batch)}
                    >
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="text-lg font-medium group-hover:text-white">{batch.name}</h3>
                      <span className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">
                        {batch.year}
                      </span>
                    </div>
                    <div className="space-y-2 text-sm text-gray-600 group-hover:text-white">
                      <p><span className="font-medium">Division:</span> {batch.division}</p>
                      <p><span className="font-medium">Subject:</span> {batch.subjectName || 'Not assigned'}</p>
                      <p><span className="font-medium">Schedule:</span> {batch.day} at {batch.time}</p>
                      <p><span className="font-medium">Duration:</span> {new Date(batch.startDate).toLocaleDateString()} - {new Date(batch.endDate).toLocaleDateString()}</p>
